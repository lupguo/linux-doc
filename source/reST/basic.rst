...
在Sphinx中，给定的文件名(这儿指 ``gnu.png`` )必须是相对于source源文件或者是相对于顶级source源文件的绝对地址。例如，文档 ``sketch/spam.rst`` 中引用图片 ``images/spam.png`` 为 ``../images/spam.png`` 或者 ``/images/spam.png``

Sphinx 将自动的复制图片到输出的构建目录(例如 ``_static``指令用于HTML输出 )

图片尺寸解释，图片的大小选项 (``width`` 和 ``height``) 如下：
   如果图片尺寸没有单位或者单位为px像素，输出大小将直接支持；
   如果是其他尺寸单位，比如 ``pt`` 或 ``bp`` ，那么将被用于html和LaTex输出(the latter replaces ``pt`` by
